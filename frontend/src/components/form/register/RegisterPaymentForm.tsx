...
type RegisterPaymentFormData = z.infer<typeof registerPaymentFormSchema>;
        ...
        <>
            <div className="p-4 mb-4 border rounded-lg bg-muted/50">
                ...
                <div className="space-y-1 text-sm">
                    <div className="flex justify-between"><span>Total da Comanda:</span> <span className="font-semibold">{formatCurrency(paymentInfo.totalAmount)}</span></div>
                    <div className="flex justify-between text-green-600"><span>Total Pago:</span> <span className="font-semibold">{formatCurrency(totalPaid)}</span></div>
                    <Separator className="my-2" />
                    <div className={`flex justify-between font-bold ${remainingAmount > 0 ? 'text-red-600' : 'text-blue-600'}`}><span>Restante:</span> <span>{formatCurrency(remainingAmount)}</span></div>
                </div>
            </div>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                                    ...
                                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                        <FormControl>
                                                            <SelectTrigger disabled={isLoadingMethods}>
                                                                <SelectValue placeholder={isLoadingMethods ? "Carregando..." : "Selecione um método"} />
                                                            </SelectTrigger>
                                                        </FormControl>
                                                        <SelectContent>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        <Button type="button" variant="outline" onClick={handleAddMore} className="w-full" disabled={remainingAmount <= 0}>
